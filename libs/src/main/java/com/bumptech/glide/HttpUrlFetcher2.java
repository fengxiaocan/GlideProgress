 ...
 */
public class HttpUrlFetcher2 implements DataFetcher<InputStream>{
    ...
    public HttpUrlFetcher2(GlideUrl glideUrl,int timeout){
        this(glideUrl,timeout,DEFAULT_CONNECTION_FACTORY);
    }

    @VisibleForTesting
    HttpUrlFetcher2(GlideUrl glideUrl,int timeout,HttpUrlConnectionFactory connectionFactory){
    ...
    private InputStream getStreamForSuccessfulRequest(HttpURLConnection urlConnection) throws IOException{
        ...
        if(TextUtils.isEmpty(urlConnection.getContentEncoding())){
            ...
            stream=GlideProgressInputStream.obtain(urlConnection.getInputStream(),contentLength,glideUrl);
        } else{
            if(Log.isLoggable(TAG,Log.DEBUG)){
                Log.d(TAG,"Got non empty content encoding: "+urlConnection.getContentEncoding());
            }
            stream=GlideNoProgressInputStream.obtain(urlConnection.getInputStream());
        }
        return stream;
    }
